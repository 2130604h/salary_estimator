...
def create_dataset_from_dataframe(
    ...
    # "Salary"以外の列を入力にする
    input = torch.tensor(df.drop(target_tag, axis=1).values, dtype=torch.float32)
    return input, target
    ...
    for epoch in range(10000):
        ...
        if epoch % 1000 == 0:
            with torch.inference_mode():  # 推論モード（学習しない）
                ...
                print(f"epoch {epoch:4d}  val MSE: {avg_val_loss:.6f}")


# --- 実行部分 ---
hitters_data = read_data()
input_tensor, target_tensor = create_dataset_from_dataframe(hitters_data)

# NNのオブジェクトを作成
nn_model = FourLayerNN(input_tensor.shape[1], 30, 1)
train_model(nn_model, input_tensor, target_tensor)

# 学習後のモデルのテスト
# (例：中央値を使ったテストデータを１件作成)
